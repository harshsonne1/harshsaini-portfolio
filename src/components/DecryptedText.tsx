...
// DecryptedText — scramble/reveal text effect (React Bits). Ported to TS; the
// original wrapped in motion.span but used no motion props, so it's a plain
// span here (no extra dependency).
import {
  useEffect,
  useState,
  useRef,
...
export default function DecryptedText({
  ...
  // auto-scramble on a repeating interval — reads the latest trigger via ref so
  // the timer isn't torn down every time isAnimating changes
  const triggerRef = useRef(triggerHoverDecrypt);
  triggerRef.current = triggerHoverDecrypt;
  useEffect(() => {
