...
// GradualBlur — progressive backdrop blur (React Bits, by Ansh Dhanani —
// github.com/ansh-dhanani). Ported to TypeScript for this project.
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      else if (w <= 768 && config[`tablet${cap}`])
        v = config[`tablet${cap}`] as string;
      else if (w <= 1024 && config[`desktop${cap}`])
        v = config[`desktop${cap}`] as string;
      setValue(v);
    };
